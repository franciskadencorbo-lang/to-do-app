...
function renderCalendarView(list = getFilteredSortedTasks()) {
  ...
  for (let w = 0; w < weekCount; w++) {
    ...
    for (let i = weekStart; i <= weekEnd; i++) {
      ...
      // off the calendar as soon as they're checked off.
      const totalMins = activeDayTasks.reduce((sum, t) => sum + taskRemainingMinutes(t), 0);
      ...
      // Tooltip: active tasks first, then completed with strikethrough
      const tooltipTasksHtml = [...activeDayTasks, ...completedDayTasks].map((t) => {
        const mins = taskRemainingMinutes(t);
        return `<div class="cal-tooltip-task${t.completed ? ' cal-tooltip-done' : ''}">
          <span class="cal-flag flag-${t.priority}">⚑</span>
          <span class="cal-tooltip-name">${escapeHtml(t.title)}</span>
          ${mins ? `<span class="cal-tooltip-time">(${formatMinutes(mins)})</span>` : ''}
        </div>`;
      }).join('');
      ...
      const summaryHtml = dayTasks.length ? `
        <div class="cal-day-summary">
          ...
          <div class="cal-view-tasks-wrap">
            <span class="cal-view-tasks">View Tasks
              <div class="cal-tasks-tooltip">${tooltipTasksHtml}</div>
            </span>
          </div>
        </div>
      ` : '';
